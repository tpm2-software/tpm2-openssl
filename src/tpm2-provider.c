...
tpm2_self_test(void *provctx)
{
    ...
    if (!CRYPTO_THREAD_write_lock(cprov->esys_lock))
        return 0;
    r = Esys_SelfTest(cprov->esys_ctx,
                      ESYS_TR_NONE, ESYS_TR_NONE, ESYS_TR_NONE,
                      TPM2_YES);
    CRYPTO_THREAD_unlock(cprov->esys_lock);
...
tpm2_teardown(void *provctx)
{
    ...
    CRYPTO_THREAD_lock_free(cprov->esys_lock);
                   ...
                   void **provctx)
{
    ...
    if ((cprov->esys_lock = CRYPTO_THREAD_lock_new()) == NULL)
        goto err3;
