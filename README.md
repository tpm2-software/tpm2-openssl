...
# Provider for integration of TPM 2.0 to OpenSSL 3+

Makes the TPM 2.0 accessible via the standard OpenSSL API and command-line tools,
so one can add TPM support to (almost) any OpenSSL 3+ based application.
...
* Implements a
  ...
  operations to the [OpenSSL 3](https://www.openssl.org/docs/OpenSSL300Design.html),
...
* Follows the new OpenSSL provider API and strictly avoids any legacy API.
  ...
  - Is compatible with OpenSSL 3.x, 4.x and (hopefully) future OpenSSL versions.
