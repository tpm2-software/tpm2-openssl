...
# generate an EC key outside the TPM
openssl genpkey -algorithm EC -pkeyopt group:P-256 -out testkey.priv

# create the NULL hierarchy primary key
tpm2_createprimary -C n -c primary.ctx

# import the generated key to the TPM
tpm2_import -C primary.ctx -G ecc -i testkey.priv -u key.pub -p abc -r key.priv
...
openssl ec -provider tpm2 -provider base -in testkey.ctx -passin pass: -check -text -noout
...
openssl ec -pubin -in testkey.pub -text -noout
