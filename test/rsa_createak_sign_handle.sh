...
openssl pkeyutl -provider tpm2 -provider base -inkey handle:${HANDLE} -sign -rawin -in testdata -out testdata.sig

# export public key
openssl pkey -provider tpm2 -provider base -in handle:${HANDLE} -pubout -out testkey.pub
