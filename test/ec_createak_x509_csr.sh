...
openssl req -provider tpm2 -provider base -new -config testcert.conf -key handle:${HANDLE} -out testcsr.pem

# display private key info
openssl pkey -provider tpm2 -provider base -in handle:${HANDLE} -text -noout
